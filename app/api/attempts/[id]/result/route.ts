...
export async function GET(req: NextRequest, { params }: { params: { id: string } }) {
  ...
  if (mock?.leaderboard_enabled) {
    ...
    // Percentile: percentage of students who scored LESS than this student
    if (totalStudents > 1 && attempt.score !== null) {
      const myScore = attempt.score;
      const scoredLower = sorted.filter(([, score]) => score < myScore).length;
      percentile = Math.round((scoredLower / totalStudents) * 100);
    }
  }
