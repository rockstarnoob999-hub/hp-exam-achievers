  ...
  if (session.role === "student") {
    ...
    if (!assigned) {
      return NextResponse.json(
        { error: "Not assigned to this test" },
        { status: 403 }
      );
    }

    const safeQuestions = (questions || []).map(
      ({ correct_option, explanation, ...q }) => q
    );
...
export async function PATCH(
  ...
  if (body.negative_marking !== undefined)
    update.negative_marking = body.negative_marking;
  if (body.passing_marks !== undefined)
    update.passing_marks = body.passing_marks;
  if (body.access_password !== undefined)
    update.access_password = body.access_password;
  if (body.instructions !== undefined)
    update.instructions = body.instructions;
  if (body.show_result_immediately !== undefined)
    update.show_result_immediately = body.show_result_immediately;
  if (body.show_correct_answers !== undefined)
    update.show_correct_answers = body.show_correct_answers;
  if (body.leaderboard_enabled !== undefined)
